 ...
 */
 ...
 class Solution {
     public static long[] nextLargerElement(long[] arr, int n) {
        ...
        long[] nextgreaterEleArr = new long[arr.length];

        st.push(arr[arr.length - 1]);
        nextgreaterEleArr[arr.length - 1] = -1; // always
        ...
        for (int i = arr.length - 2; i >= 0; i--) {
            ...
            if (st.size() == 0) {
                nextgreaterEleArr[i] = -1;
            } else {
                nextgreaterEleArr[i] = st.peek();
            }
            st.push(arr[i]);
        }

        return nextgreaterEleArr;
    }
}